...
etable(list(
  feols(log(totalinc) ~ i(sex, provtariff) | hhid02 + year,
        data = inc0402_p,
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(log(totalinc) ~ i(sex, provtariff_k) | hhid02 + year,
        data = inc0402_p,
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(log(totalinc) ~ i(sex, provtariff) | hhid06 + year,
        data = inc0602_p,
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(log(totalinc) ~ i(sex, provtariff_k) | hhid06 + year,
        data = inc0602_p,
        weights = ~hhwt, 
        vcov = ~tinh)  
))
...
etable(list(
  feols(log(totalinc) ~ i(sex, provtariff) | ivid02 + year,
        data = inc0402_p,
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(log(totalinc) ~ i(sex, provtariff_k) | ivid02 + year,
        data = inc0402_p,
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(log(totalinc) ~ i(sex, provtariff) | ivid02 + year,
...
etable(list(
  feols(inc_ratio ~ provtariff/sex | hhid02 + year,
        ...
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(inc_ratio ~ sex*provtariff_k | hhid02 + year,
        inc_0204_spouse_p,
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(inc_ratio ~ sex*provtariff| hhid06 + year,
        inc_0206_spouse_p,
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(inc_ratio ~ sex*provtariff_k | hhid06 + year,
        inc_0206_spouse_p,
        weights = ~hhwt, 
        vcov = ~tinh)  
), tex = TRUE)
...
etable(list(
  feols(inc_ratio ~ i(sex, provtariff) | hhid02 + year,
        inc_0204_spouse_p,
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(inc_ratio ~ i(sex, provtariff_k) | hhid02 + year,
        inc_0204_spouse_p,
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(inc_ratio ~ i(sex, provtariff) | hhid06 + year,
        inc_0206_spouse_p,
        weights = ~hhwt, 
        vcov = ~tinh),
  feols(inc_ratio ~ i(sex, provtariff_k) | hhid06 + year,
        inc_0206_spouse_p,
        weights = ~hhwt, 
        vcov = ~tinh)  
), tex = TRUE)
