...
etable(list(
  feols(work ~ i(as.factor(Female), provtariff) | ivid + year,
        ...
        vcov = ~tinh,
        weights = ~hhwt),
  feols(work ~ i(as.factor(Female), provtariff_k) | ivid + year,
        employment0204_p,
        vcov = ~tinh,
        weights = ~hhwt),
  feols(work ~ i(as.factor(Female), provtariff) | ivid02 + year,
        employment0206_p,
        vcov = ~tinh,
        weights = ~hhwt),
  feols(work ~ i(as.factor(Female), provtariff_k) | ivid02 + year,
        employment0206_p,
        vcov = ~tinh,
        weights = ~hhwt)  
), tex = TRUE)
